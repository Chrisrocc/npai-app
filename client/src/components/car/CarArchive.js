  ...
  const CarArchive = () => {
    ...
    useEffect(() => {
      const fetchArchivedCars = async () => {
        try {
          const response = await axios.get('http://localhost:5000/api/cars/archived');
    ...
    const handleRestore = async (id) => {
      ...
      try {
        const response = await axios.post(`http://localhost:5000/api/cars/${id}/restore`);
    ...
    const handlePermanentDelete = async (id) => {
      ...
      try {
        const response = await axios.delete(`http://localhost:5000/api/cars/${id}/permanent`);
